...
Now that we understand our data structure better, we will calculate the mean surface temperature for the seven day for which we have data, and create a map.  
...
Now the `NA` values show as a bright pink, and there is much less of these grid cells, which makes sense. However, bright pink is probably not the best choice of colour, but it highlights them in our example and  illustrates how to do this.  
...
```{r}
#Calculating mean temperature per timestep
...
We got a line plot with mean temperatures over time for each depth bin in the model. We can save this plot using the `ggsave` function.  
