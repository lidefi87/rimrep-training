...
This particular dataset contains a variable called `geometry`. It is an add-on that the DMS created to allow you to extract data using specific geometry filters (like bounding boxes). If you don't plan to use the geometry field, it is better to filter it out at the beginning of any query, so the extraction of the data will be more efficient 
...
large_fishes_23 <- data_df |> 
  ...
  select(!geometry) |> 
  #Note that we need to write the variable exactly as it is showing above
  filter(variable == "Large fishes") |> # & reef_zone == "North") |> 
  #Turn the string to date type
   mutate(date = ymd(date), 
